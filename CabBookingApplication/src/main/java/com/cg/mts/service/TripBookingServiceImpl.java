...
public class TripBookingServiceImpl implements ITripBookingService {
	...
	/*
	 * @Autowired private IDriverRepository driverRepository;
	 * 
	 * @Autowired private ICabRepository cabRepository;
	 */
	...
	public TripBooking updateTripBookingByUserId(TripBooking tripBooking, int tripBookingId, int userId) {
		...
		/*
		 * if( savedTripBooking != null & savedUser !=null) { TripBooking
		 * updatedTripBooking =
		 * tripBookingRepository.updateTripBookingByUserId(savedTripBooking);
		 * tripBookingRepository.save(updatedTripBooking); // if data is already saved
		 * then hibernate calls update query in background return updatedTripBooking; }
		 * else return null;
		 */
		return null;
	}
