...
public class TripBookingRestController {
	...
	@PostMapping("/register/{userId}/{driverId}")
	...
	@GetMapping("/alltrips")
	public ResponseEntity<List<TripBookingDTO>> getAllTrips() throws InvalidUserException
	{
		List<TripBooking> allTripsInDB = tripBookingService.viewAllTrips();
	...
	@GetMapping("/alltripsbydriver/{id}")
	public ResponseEntity<List<TripBookingDTO>> getAllTripsByDriver( @PathVariable("id") int driverId) throws InvalidUserException
	{
		List<TripBooking> allTripsInDB = tripBookingService.viewAllTripsDriver(driverId);
		
		List<TripBookingDTO> dtoListdriver = new ArrayList<>();
		for (TripBooking tripBooking : allTripsInDB) {
			
			TripBookingDTO dtoObj = dtoConvertor.convertTo(tripBooking);
			dtoListdriver.add(dtoObj);
		}
		
		return new ResponseEntity<List<TripBookingDTO>>(dtoListdriver,HttpStatus.OK);
	}
