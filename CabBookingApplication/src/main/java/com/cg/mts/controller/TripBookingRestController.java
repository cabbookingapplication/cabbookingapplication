...
public class TripBookingRestController {
	...
	public ResponseEntity<TripBookingDTO> saveTripBooking(@RequestBody TripBooking tripBooking,
			@PathVariable int userId, @PathVariable int driverId) {
	...
	public String updateTripBookingByUserID(@RequestBody TripBooking tripBooking, @PathVariable int tripBookingId,
			@PathVariable int userId) throws TripBookingNotFoundException {
		...
		if (updatedTrip.getTripBookingId() != 0) {
			...
			return "error : - " + updatedTrip.toString() + " ";
	}

	/*
	 * @PutMapping("/updatetrip/{id}") public TripBooking
	 * updateTripBooking( @RequestBody TripBooking
	 * tripBooking, @PathVariable("id")int id) { return
	...
	public ResponseEntity<List<TripBookingDTO>> getAllTrips(@PathVariable("id") int userId)
